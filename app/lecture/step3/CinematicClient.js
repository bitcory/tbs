...
const CINE_TABS = [
  { id: 'overview',   label: '개요',     dot: '#a8a4b2' },
    ...
    <div className="min-h-screen md:h-screen md:flex md:flex-col md:overflow-hidden bg-[#15141a] text-[#f5f4f7]">
      <style jsx global>{`
        ...
        .tb-glass-bar { position: relative; z-index: 3; margin: -22px 16px 0;
          ...
          background: rgba(31,29,38,0.7); border: 1px solid rgba(255,255,255,0.1);
        ...
        .tb-pill-ghost { background: rgba(38,36,46,0.6); color: #f5f4f7;
        ...
        .tb-pill-ghost:hover { background: rgba(38,36,46,0.85); transform: translateY(-1px) scale(1.03); }
      ...
      <div className="tb-glass-bar">
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          {data && (
            <button
              onClick={reset}
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-[#1f1d26] border border-[#7f1d1d] text-[#f87171] text-xs sm:text-sm font-bold tb-press-soft"
            >
            ...
            {data?.project ? (
              <div className="space-y-1.5">
                ...
                {data.project.subtitle && (
                  <div className="flex items-start gap-2">
                    <span className="text-sm text-[#a8a4b2] font-medium w-14 pt-0.5">부제</span>
                    <span className="text-sm text-[#f5f4f7] font-bold flex-1 break-all">{data.project.subtitle}</span>
                  </div>
                )}
                {(data.scenes?.length > 0) && (
                  <div className="flex items-start gap-2">
                    <span className="text-sm text-[#a8a4b2] font-medium w-14 pt-0.5">씬 수</span>
                    <span className="text-sm text-[#f5f4f7] font-bold flex-1">{data.scenes.length}개</span>
                  </div>
                )}
                {(data.characters?.length > 0) && (
                  <div className="flex items-start gap-2">
                    <span className="text-sm text-[#a8a4b2] font-medium w-14 pt-0.5">캐릭터</span>
                    <span className="text-sm text-[#f5f4f7] font-bold flex-1">{data.characters.length}명</span>
                  </div>
                )}
                {(data.videoClips?.length > 0) && (
                  <div className="flex items-start gap-2">
                    <span className="text-sm text-[#a8a4b2] font-medium w-14 pt-0.5">영상</span>
                    <span className="text-sm text-[#f5f4f7] font-bold flex-1">{data.videoClips.length} 클립</span>
                  </div>
                )}
              </div>
            ) : (
          ...
          <div className="p-4 border-b border-[#34323d]">
            ...
            <div className="space-y-1">
              <a href="https://translate.google.co.kr/?sl=ko&tl=en&op=translate" target="_blank" rel="noreferrer"
                className="w-full flex items-center gap-2 px-3 py-2 rounded-full text-sm font-bold transition text-[#93c5fd] bg-[#1a2740] hover:bg-[#22335a] tb-press-soft">
                <Languages className="w-4 h-4" /> 구글번역기
              </a>
              <a href="https://tbnc.aitoolb.com/" target="_blank" rel="noreferrer"
                className="w-full flex items-center gap-2 px-3 py-2 rounded-full text-sm font-bold transition text-[#fda4af] bg-[#311a26] hover:bg-[#42202f] tb-press-soft">
...
function EmptyState({ onOpen }) {
  return (
    <div className="flex-1 flex flex-col items-center justify-center text-center p-10">
      <div className="w-20 h-20 mb-5 rounded-full flex items-center justify-center bg-[#3a2212] border border-[#34323d]">
        <Clapperboard className="w-10 h-10 text-[#f97316]" />
      </div>
      <h3 className="text-lg font-bold text-[#f5f4f7] mb-2">프로덕션 바이블이 없습니다</h3>
...
function PlaceholderCard({ label }) {
  return (
    <div className="rounded-2xl border-2 border-dashed border-[#34323d] bg-[#26242e] p-10 text-center">
      <div className="text-sm font-bold text-[#c8c4d2] mb-1">{label} 탭</div>
      <p className="text-xs text-[#a8a4b2]">JSON에 해당 정보가 있으면 여기에 표시됩니다 (Phase 2에서 구현 예정).</p>
    </div>
  );
}
...
function OverviewPlaceholder({ data }) {
  ...
  return (
    <div className="rounded-2xl border border-[#34323d] bg-[#1f1d26] p-5">
      <h3 className="text-base font-extrabold mb-3">{p.title || '프로젝트'}</h3>
      {p.subtitle && <p className="text-sm text-[#a8a4b2] mb-3">{p.subtitle}</p>}
      <p className="text-xs text-[#a8a4b2]">개요 탭의 풍부한 표시는 Phase 2에서 구현 예정입니다. 지금은 [이미지] 탭에서 씬별 프롬프트를 확인하세요.</p>
    </div>
  );
}
...
function CharactersTab({ characters, onCopy, copiedId }) {
  if (!characters || characters.length === 0) {
    return (
      <div className="text-center py-12 text-sm text-[#a8a4b2]">
        등록된 캐릭터가 없습니다. JSON에 <code className="px-1.5 py-0.5 rounded bg-[#26242e] text-[#c8c4d2]">characters</code> 배열 또는 씬의 <code className="px-1.5 py-0.5 rounded bg-[#26242e] text-[#c8c4d2]">scene.characters</code>가 포함되어야 합니다.
      </div>
    );
  }
  return (
    <div className="space-y-4">
      {characters.map((c, i) => {
        ...
        return (
          <div key={i} className="rounded-2xl overflow-hidden border-2 border-[#34323d] bg-[#1f1d26]">
            <div className="px-4 py-2.5 border-b-2 border-[#34323d] flex items-center justify-between gap-2 flex-wrap"
                 style={{ background: stc.bg }}>
              <div className="flex items-center gap-2 flex-wrap min-w-0">
                <User className="w-4 h-4 text-[#f5f4f7]" />
                <span className="text-sm font-bold text-[#f5f4f7] truncate">{name}</span>
                {c.nameEn && (
                  <span className="text-[11px] text-[#c8c4d2] font-medium">{c.nameEn}</span>
                )}
                {c.age && (
                  <span className="text-[10px] uppercase tracking-wider font-bold px-2 py-0.5 rounded bg-white/10 text-[#c8c4d2] border border-[#f5f4f7]/15">
                    {c.age}
                  </span>
                )}
                {c.role && (
            ...
            {c.description && (
              <div className="px-4 py-2 text-xs text-[#c8c4d2] border-b border-[#34323d] leading-relaxed">
                {c.description}
              </div>
            )}
            ...
            <div className="p-4 space-y-3">
              {c.outfit && (
                <div className="rounded-lg border border-[#34323d] bg-[#1f1d26] overflow-hidden">
                  <div className="px-3 py-1.5 bg-[#26242e] border-b border-[#34323d]">
                    <span className="text-[10px] font-bold text-[#c8c4d2] uppercase tracking-wider">의상</span>
                  </div>
                  <div className="p-3 text-sm text-[#f5f4f7] leading-relaxed">{c.outfit}</div>
                </div>
              )}
              ...
              {appearance && Object.keys(appearance).length > 0 && (
                <div className="rounded-lg border border-[#34323d] bg-[#1f1d26] overflow-hidden">
                  <div className="px-3 py-1.5 bg-[#26242e] border-b border-[#34323d]">
                    <span className="text-[10px] font-bold text-[#c8c4d2] uppercase tracking-wider">외형</span>
                  </div>
                  <dl className="p-3 grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-1.5 text-sm">
                    {Object.entries(appearance).map(([k, v]) => (
                      <div key={k} className="flex gap-2 min-w-0">
                        <dt className="text-[#a8a4b2] font-bold shrink-0 w-16 capitalize">{k}</dt>
                        <dd className="text-[#f5f4f7] truncate" title={String(v)}>{String(v)}</dd>
                      </div>
                    ))}
                  </dl>
                </div>
              ...
              {promptText && (
                <div className="rounded-lg border-2 border-[#34323d] overflow-hidden bg-[#2a2616]">
                  <div className="px-3 py-1.5 bg-[#26242e] flex items-center justify-between border-b border-[#34323d]">
                    <span className="text-[10px] font-bold text-[#c8c4d2] uppercase tracking-wider">캐릭터 프롬프트</span>
                    <CopyBtn text={promptText} id={`char-${i}`} label="EN 복사" onCopy={onCopy} copiedId={copiedId} />
                  </div>
                  <div className="p-3 text-sm text-[#f5f4f7] leading-relaxed font-mono whitespace-pre-wrap">{promptText}</div>
                </div>
              )}
            </div>
          </div>
...
function ScenesTab({ scenes, sceneIdx, setSceneIdx, onCopy, copiedId }) {
  if (scenes.length === 0) {
    return (
      <div className="text-center py-12 text-sm text-[#a8a4b2]">
        등록된 씬이 없습니다. JSON에 <code className="px-1.5 py-0.5 rounded bg-[#26242e] text-[#c8c4d2]">scenes</code> 배열을 포함하세요.
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
          ...
          return (
            <button
              ...
              className={`shrink-0 px-4 py-2 text-sm font-bold whitespace-nowrap transition rounded-lg border-2 ${
                isActive ? 'border-[#f97316] shadow-md' : 'border-[#34323d] hover:border-[#a8a4b2]'
              } flex items-center gap-2`}
              style={{ background: isActive ? stc.bg : '#1f1d26', color: '#f5f4f7' }}
            >
      ...
      {/* Scene header card */}
      <div className="rounded-2xl overflow-hidden border-2 border-[#34323d] bg-[#1f1d26]">
        <div className="px-4 py-3 border-b-2 border-[#34323d]" style={{ background: headerColor.bg }}>
          <div className="flex items-center justify-between gap-3 flex-wrap">
            <div className="text-base font-black text-[#f5f4f7]">{sceneId}. {s.title || ''}</div>
        ...
        {s.koreanRef && (
          <div className="px-4 py-2.5 border-b border-[#34323d] text-sm text-[#c8c4d2] leading-relaxed">{s.koreanRef}</div>
        )}
        {s.bgPrompt && (
          <div className="p-4">
            ...
            <div className="p-3 rounded-lg border-2 border-[#34323d] bg-[#2a2616] text-sm text-[#f5f4f7] leading-relaxed font-mono whitespace-pre-wrap">{s.bgPrompt}</div>
            {s.bgPromptKr && <div className="mt-2 text-xs text-[#a8a4b2] leading-relaxed">{s.bgPromptKr}</div>}
          </div>
        )}
      </div>
      ...
      {(s.shots || []).map((shot, si) => {
        const stc = TAB_COLORS[si % TAB_COLORS.length];
        return (
          <div key={si} className="rounded-2xl overflow-hidden border-2 border-[#34323d] bg-[#1f1d26]">
            <div className="px-4 py-2.5 border-b-2 border-[#34323d] flex items-center justify-between gap-2 flex-wrap"
                 style={{ background: stc.bg }}>
              <div className="flex items-center gap-2 flex-wrap min-w-0">
                ...
                {shot.label && <span className="text-sm font-bold text-[#f5f4f7] truncate">{shot.label}</span>}
              </div>
              {shot.category && (
                <span className="text-[10px] px-2 py-0.5 rounded font-bold border border-[#f5f4f7]/15 bg-white/10 text-[#c8c4d2]">
                  {shot.category}
                </span>
              )}
            </div>
            {shot.note && (
              <div className="px-4 py-2 text-xs text-[#c8c4d2] border-b border-[#34323d]">{shot.note}</div>
            )}
            <div className="p-4 space-y-3">
              {(shot.prompts || []).map((pr, pi) => (
                <div key={pi} className="rounded-lg border-2 border-[#34323d] overflow-hidden bg-[#2a2616]">
                  <div className="px-3 py-1.5 bg-[#26242e] flex items-center justify-between border-b border-[#34323d]">
                    <span className="text-[10px] font-bold text-[#c8c4d2] uppercase tracking-wider">{pr.tag}</span>
                    <CopyBtn text={pr.en} id={`shot-${safeIdx}-${si}-${pi}`} label="EN 복사" onCopy={onCopy} copiedId={copiedId} />
                  </div>
                  <div className="p-3 text-sm text-[#f5f4f7] leading-relaxed font-mono whitespace-pre-wrap">{pr.en}</div>
                  {pr.kr && (
                    <div className="px-3 py-2 text-xs text-[#a8a4b2] leading-relaxed bg-[#26242e] border-t border-[#34323d]">{pr.kr}</div>
                  )}
                </div>
              ))}
            </div>
...
function Badge({ children, soft }) {
  return (
    <span className={`text-[10px] px-2 py-0.5 rounded${soft ? '-full' : ''} font-bold border border-[#f5f4f7]/15 ${soft ? 'bg-[#26242e]' : 'bg-white/10'}`}>
      {children}
    </span>
  );
}
    ...
    <button
      ...
      className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-md text-[11px] font-bold border-2 transition disabled:opacity-40 ${
        isCopied
          ...
          : 'border-[#34323d] text-[#f5f4f7] bg-[#26242e] hover:bg-[#34323d]'
      }`}
    >
    ...
    <div onClick={onClose} className="fixed inset-0 z-[300] flex items-center justify-center bg-black/70 backdrop-blur-md p-4">
      <div onClick={(e) => e.stopPropagation()}
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-[#34323d]">
          <span className="text-base font-bold text-[#f5f4f7] uppercase tracking-wider">프로덕션 바이블 JSON 업로드</span>
          <button onClick={onClose}
            className="w-8 h-8 flex items-center justify-center rounded-full bg-[#26242e] hover:bg-[#34323d] text-[#c8c4d2] tb-press-soft">
            <X className="w-4 h-4" />
          </button>
        </div>
